...
<div id="reviews" class="woocommerce-Reviews">
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
     ...
     <div class="input-group-label" id="name-span">
        <i class="fa fa-user"></i></div>
      <input class="product_review_author" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" placeholder="' . esc_html__( 'Name', 'woocommerce' ) .'" aria-required="true" required /></div>' /*</p>*/ ,
							'email'  =>  /*<p class="comment-form-email">' . '<label for="email">' . esc_html__( 'Email', 'woocommerce' ) . ' <span class="required">*</span></label> ' .*/
										'<div class="review-input-group">
      <div class="input-group-label" id="email-span">
        <i class="fa fa-envelope"></i>
      </div>
<input class="product_review_email" id="email" name="email" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_html__( 'Email', 'woocommerce' ) . '" aria-required="true" required /></div>' ,
						),
    ...
    jQuery("#name-span").css("color", "#eb286e");});
	jQuery("#author").focusout(function(){
    jQuery("#name-span").css("color", "#46a9c6");});
	jQuery("#email").focus(function(){
    jQuery("#email-span").css("color", "#eb286e");});
	jQuery("#email").focusout(function(){
    jQuery("#email-span").css("color", "#46a9c6");}); </script>
	<?php else : ?>
