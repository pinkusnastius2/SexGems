...
function ecommerce_gem_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'ecommerce-gem' ),
		'id'            => 'sidebar-1',
...
function limit_tag_in_tag_cloud_widget($args){
	if(isset($args['taxonomy']) && $args['taxonomy'] || 'post_tag'&& $args['taxonomy'] == 'product_tag'){
...
if( !is_admin() ){ // not admin side
    add_filter( 'the_content', 'strip_spans' );
	add_filter( 'woocommerce_template_single_excerpt', 'strip_spans' );
}

function strip_spans( $content )
{
    return strip_tags( $content, '<p><br><div><strong><h><h1><h2><h3><h4><h5><a>' );
}